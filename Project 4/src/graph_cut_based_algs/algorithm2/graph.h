...
public:
	...
	// Adding only source-link or sink-link for each node.
	// issink=1----sink-link; issink=0-----sourcelink;
	//Modifed by Jun Liu, Jul. 2012.
	void add_tweights(node_id i, tcaptype cap, bool issink);
...
// This is the function that only adding source link or sink link.
// Modeified by JunLiu, Jul. 2012.
template <typename captype, typename tcaptype, typename flowtype> 
	inline void Graph<captype,tcaptype,flowtype>::add_tweights(node_id i, tcaptype cap, bool issink)
{
	assert(i >= 0 && i < node_num);

	tcaptype delta = nodes[i].tr_cap;
	tcaptype cap_source=(tcaptype)(0.0);
	tcaptype cap_sink=(tcaptype) (0.0);
	if (!issink)
	{
	   if (delta>0) cap_source=cap+delta;
	   else
	   {
		  cap_sink=-delta;
		  cap_source=cap;
